...
//   if (!name) return response.status(400).json({ message: 'O campo "name" é obrigatório' });
//   if (name.length <= 3) return response.status(400)
// .json({ message: 'O "name" deve ter pelo menos 3 caracteres' });

//   if (!age || age === '') return response.status(400)
...
//   }
//   if (date.rate < 1 || date.rate > 5) return response.status(400)
// .json({ message: 'O campo "rate" deve ser um inteiro de 1 à 5' });
//   if (dateValid(date.datedAt)) return response.status(400)
...
//   crushesWithoutId.concat(crushEdited);
