...
app.get('/crush', async (_req, res) => {
  const chushes = await fs.readFile(crushFile);
  if (!chushes) {
    return res.status(200).json([]);
  }
  return res.status(200).json(JSON.parse(chushes));
});

app.get('/crush/:id', async (req, res) => {
  const chushes = await fs.readFile(crushFile, 'utf-8');
  const file = JSON.parse(chushes);
  const { id } = req.params;
...
app.post('/login', (req, res) => {
  ...
  if (emailValidation(email) === false) return res.status(400).json({ message: 'O "email" deve ter o formato "email@email.com"' });
  if (!password) return res.status(400).json({ message: 'O campo "password" é obrigatório' });
  if (passwordValidation(password) < 6) return res.status(400).json({ message: 'A "senha" deve ter pelo menos 6 caracteres' });
