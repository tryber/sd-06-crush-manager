...
function verifyCrush(request, response, next) {
  ...
  if (age === undefined) return response.status(400).json({ message: 'O campo "age" é obrigatório' });
  ...
  if (!date || !date.datedAt || date.rate === undefined) return response.status(400).json({ message: 'O campo "date" é obrigatório e "datedAt" e "rate" não podem ser vazios' });
