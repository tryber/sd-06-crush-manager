...
async function searchCrush(req, res) {
  ...
  if (String(name).length < 3) return res.status(400).json({ message: 'O "name" deve ter pelo menos 3 caracteres' });
  ...
  // // if (!date || date === '' || datedAt === '' || rate === '' || !datedAt || !rate)
  // // return res.status(400).
  // json({ message: 'O campo "date" é obrigatório e "datedAt" e "rate" não podem ser vazios' });

  if (!date || !datedAt || !rate) return res.status(400).json({ message: 'O campo "date" é obrigatório e "datedAt" e "rate" não podem ser vazios' });

  if (!regexDate.test(String(datedAt))) return res.status(400).json({ message: 'O campo "datedAt" deve ter o formato "dd/mm/aaaa"' });
